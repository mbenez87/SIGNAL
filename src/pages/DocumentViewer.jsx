            ...
            <div className="flex items-center gap-1 sm:gap-2 md:gap-4 flex-1 min-w-0">
              ...
              {document.file_type === 'pdf' && (
                <div className="hidden lg:flex items-center gap-2 mr-2 lg:mr-4">
                  <Button variant="outline" size="icon" className="h-10 w-10 lg:h-12 lg:w-12" onClick={() => setZoom(Math.max(50, zoom - 10))}>
                    <ZoomOut className="w-5 h-5 lg:w-6 lg:h-6" />
                  </Button>
                  <span className="text-xs lg:text-sm text-slate-600 w-10 lg:w-12 text-center font-semibold">{zoom}%</span>
                  <Button variant="outline" size="icon" className="h-10 w-10 lg:h-12 lg:w-12" onClick={() => setZoom(Math.min(200, zoom + 10))}>
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if ((document.processing_status === 'processing' || document.processing_status === 'pending') && document.updated_date) {
                      ...
                      if (timeSinceUpdate > 60000) { console.warn('Document stuck in processing state for >60s'); }
                    }
                  }}
                >
              ...
              {!isFullscreen && (
                <>
                  ...
                  <Button variant="outline" size="icon" className="h-8 w-8 sm:h-9 sm:w-9 md:h-10 md:w-10 lg:h-12 lg:w-12 hidden sm:flex" onClick={() => setShowShareModal(true)}>
                    <Share2 className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6" />
                  </Button>

                  <Button variant="outline" size="icon" className="h-8 w-8 sm:h-9 sm:w-9 md:h-10 md:w-10 lg:h-12 lg:w-12 hidden md:flex" onClick={handleDelete}>
          ...
          <div className={`${isFullscreen ? 'flex-1 bg-slate-900' : 'flex-1 bg-white rounded-lg shadow-lg'} overflow-hidden h-full`}>
            {document.file_type === 'pdf' ? (
              ...
              <div className={`flex items-center justify-center ${isFullscreen ? 'h-full' : 'p-8'}`}>
                <img src={document.file_url} alt={document.title} className={`${isFullscreen ? 'max-h-full max-w-full object-contain' : 'max-w-full h-auto'}`} />
              </div>
            ) : (
